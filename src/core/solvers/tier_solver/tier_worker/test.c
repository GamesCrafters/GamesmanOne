...
static bool TestChildTiers(TierPosition parent,
    ...
    TierHashSet child_tiers;
    TierHashSetInit(&child_tiers, 0.5);
    TierArray child_tiers_array = api_internal->GetChildTiers(parent.tier);

    // This should be removed once the new API is done.
    TierHashSetAdd(&child_tiers, parent.tier);
    for (int64_t i = 0; i < child_tiers_array.size; ++i) {
        TierHashSetAdd(&child_tiers, child_tiers_array.array[i]);
    }
    TierArrayDestroy(&child_tiers_array);
    for (int64_t i = 0; i < children->size; ++i) {
        if (!TierHashSetContains(&child_tiers, children->array[i].tier)) {
...
static int TestChildPositions(Tier tier, Position position) {
    ...
    // Test if the child tier positions' tiers are actually from the tiers
    // generated by the TierSolverApi::GetChildTiers function.
    if (!TestChildTiers(parent, &children)) {
