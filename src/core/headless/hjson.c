...
int HeadlessJsonAddRemoteness(json_object *dest, int remoteness) {
    ...
    if (remoteness_obj == NULL) return 1;
...
int HeadlessJsonAddError(json_object *dest, ReadOnlyString message) {
    json_object *error_obj = json_object_new_string(message);
    if (error_obj == NULL) return 1;

    return json_object_object_add(dest, "error", error_obj);
}
