...
static int KaooaFinalize(void);

static const GameVariant *KaooaGetCurrentVariant(void);
static int KaooaSetVariantOption(int option, int selection);

static Tier KaooaGetInitialTier(void);
...
static Position KaooaGetCanonicalPosition(TierPosition tier_position);
static int KaooaGetNumberOfCanonicalChildPositions(TierPosition tier_position);
static TierPositionArray KaooaGetCanonicalChildPositions(
    TierPosition tier_position);
static PositionArray KaooaGetCanonicalParentPositions(
    TierPosition tier_position, Tier parent_tier);
static TierArray KaooaGetChildTiers(Tier tier);
static int KaooaGetTierName(char *name, Tier tier);
...
static TierPosition KaooaFormalPositionToTierPosition(
    ReadOnlyString formal_position);
static CString KaooaTierPositionToFormalPosition(TierPosition tier_position);
static CString KaooaTierPositionToAutoGuiPosition(TierPosition tier_position);
static CString KaooaMoveToFormalMove(TierPosition tier_position, Move move);
static CString KaooaMoveToAutoGuiMove(TierPosition tier_position, Move move);

// Variants

static ConstantReadOnlyString kKaooaRuleChoices[] = {
    "Crows and Vultures"
};

static const GameVariantOption kKaooaRules = {
    .name = "rules",
    .num_choices = sizeof(kKaooaRuleChoices) / sizeof(kKaooaRuleChoices[0]),
    .choices = kKaooaRuleChoices,
};

#define NUM_OPTIONS 2  // 1 option and 1 zero-terminator
static GameVariantOption Kaooa_variant_options[NUM_OPTIONS];
static int Kaooa_variant_option_selections[NUM_OPTIONS] = {0, 0};  // 5x5
#undef NUM_OPTIONS
...
static const TierSolverApi kKaooaSolverApi = {
    ...
    .GetCanonicalPosition = &KaooaGetCanonicalPosition,
    .GetNumberOfCanonicalChildPositions =
        &KaooaGetNumberOfCanonicalChildPositions,
    .GetCanonicalChildPositions = &KaooaGetCanonicalChildPositions,
    .GetCanonicalParentPositions = &KaooaGetCanonicalParentPositions,
...
static const GameplayApi KaooaGameplayApi = {
...
static const UwapiTier kKaooaUwapiTier = {
    ...
    .FormalPositionToTierPosition = &KaooaFormalPositionToTierPosition,
    .TierPositionToFormalPosition = &KaooaTierPositionToFormalPosition,
    .TierPositionToAutoGuiPosition = &KaooaTierPositionToAutoGuiPosition,
...
const Game kKaooa = {
    ...
    .formal_name = "Kaooa",
    .solver = &kTierSolver,
    .solver_api = (const void *)&kKaooaSolverApi,
    .gameplay_api = (const GameplayApi *)&KaooaGameplayApi,
    .uwapi = &kKaooaUwapi,

    .Init = &KaooaInit,
    .Finalize = &KaooaFinalize,

    .GetCurrentVariant = &KaooaGetCurrentVariant,
    .SetVariantOption = &KaooaSetVariantOption,
};
...
static Tier KaooaSetInitialTier(void);
static Position KaooaSetInitialPosition(void);
static Tier HashTier(int num_blanks, int num_x, int num_o);
static void UnhashTier(Tier tier, int *num_blanks, int *num_x, int *num_o);
static void KaooaInitSymmMatrix();

// Helper functions for KaooaGenerateMoves
...
// Helper functions for KaooaPrimitive
...
// Helper functions for KaooaDoMove and KaooaGetCanonicalParentPositions
static int GetShiftOffset(int src, int dest);

static int KaooaInit(void *aux) {
    (void)aux;  // Unused.
    Kaooa_variant_options[0] = kKaooaRules;
    return KaooaSetVariantOption(0, 0);  // Initialize the default variant.
}
...
static int KaooaInitVariant(int rows, int cols, int k) {
    board_rows = rows;
    board_cols = cols;
    k_in_a_row = k;
    UpdateEdgeSlots();
...
static int KaooaSetVariantOption(int option, int selection) {
    ...
    if (option != 0 || selection < 0 || selection >= kKaooaRules.num_choices) {
        return kRuntimeError;
    }

    Kaooa_variant_option_selections[0] = selection;
    switch (selection) {
        case 0:  // 5x5 5-in-a-row
            return KaooaInitVariant(5, 5, 5);
        case 1:  // 4x4 4-in-a-row
            return KaooaInitVariant(4, 4, 4);
        case 2:  // 3x3 3-in-a-row
            return KaooaInitVariant(3, 3, 3);
        case 3:  // 2x2 2-in-a-row
            return KaooaInitVariant(2, 2, 2);
        case 4:  // 4x5 4-in-a-row
            return KaooaInitVariant(4, 5, 4);
        default:
...
static Tier KaooaGetInitialTier(void) { return initial_tier; }

static Position KaooaGetInitialPosition(void) { return initial_position; }
...
static bool KaooaIsLegalPosition(TierPosition tier_position) {
    ...
    if (!success) {
        fprintf(stderr,
                "KaooaIsLegalPosition: GenericHashUnhashLabel error on tier "
...
static Position KaooaDoSymmetry(Tier tier, const char *original_board, int turn,
                                int symmetry) {
...
static Position KaooaGetCanonicalPosition(TierPosition tier_position) {
    ...
    for (int i = 0; i < NumSymmetries(); ++i) {
        Position new_position = KaooaDoSymmetry(tier, board, turn, i);
...
static int KaooaGetNumberOfCanonicalChildPositions(TierPosition tier_position) {
    ...
    for (int i = 0; i < num_edge_slots; ++i) {
        ...
        for (int j = 0; j < edge_move_count[i]; ++j) {
            ...
            child.position = KaooaGetCanonicalPosition(child);
...
static PositionArray KaooaGetCanonicalParentPositions(
    ...
    if (!success) {
        fprintf(
            stderr,
            "KaooaGetCanonicalParentPositions: GenericHashUnhashLabel error\n");
        return parents;
    }
    ...
    for (int i = 0; i < num_edge_slots; ++i) {
        ...
        for (int j = 0; j < edge_move_count[i]; ++j) {
            ...
            parent.position = KaooaGetCanonicalPosition(parent);
...
static int KaooaGetTierName(char *name, Tier tier) {
    ...
    if (actual_length >= kDbFileNameLengthMax + 1) {
        fprintf(stderr, "KaooaGetTierName: (BUG) tier db filename overflow\n");
...
static int KaooaMoveToString(Move move, char *buffer) {
    ...
    int actual_length = snprintf(
        buffer, KaooaGameplayApiCommon.move_string_length_max + 1, "%d %d",
        src + 1, dest + 1);  // add 1 to src and dest for user representation
    if (actual_length >= KaooaGameplayApiCommon.move_string_length_max + 1) {
        fprintf(stderr,
                "KaooaMoveToString: (BUG) not enough space was allocated "
...
static bool KaooaIsLegalFormalPosition(ReadOnlyString formal_position) {
    ...
    if (!KaooaIsLegalPosition(tier_position)) return false;
...
static CString KaooaTierPositionToAutoGuiPosition(TierPosition tier_position) {
    ...
    CString ret = KaooaTierPositionToFormalPosition(tier_position);
...
static Position KaooaSetInitialPosition(void) {
    ...
    initial_position = GenericHashHashLabel(KaooaGetInitialTier(), board, 1);
