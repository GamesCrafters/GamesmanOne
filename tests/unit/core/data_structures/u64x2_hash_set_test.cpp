...
extern "C" {
#define U64X2_HASH_SET_SIZE TEST_SET_CAPACITY
...
TEST(U64x2HashSetTest, Initialization) {
    U64x2HashSet set;
    ...
    EXPECT_FALSE(U64x2HashSetContains(&set, key));
}

// Verifies that adding a single unique element correctly updates the set's
// state and size.
TEST(U64x2HashSetTest, AddSingleElement) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);

    U64x2 key = {0xCAFEBABE, 0xDEADBEEF};

    EXPECT_TRUE(U64x2HashSetAdd(&set, key));
    EXPECT_EQ(set.size, 1);
    EXPECT_TRUE(U64x2HashSetContains(&set, key));
}

// Verifies that the hash set correctly identifies identical SIMD vectors and
// rejects duplicate insertions.
TEST(U64x2HashSetTest, AddDuplicateElement) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);

    U64x2 key = {0xCAFEBABE, 0xDEADBEEF};

    ASSERT_TRUE(U64x2HashSetAdd(&set, key));

    EXPECT_FALSE(U64x2HashSetAdd(&set, key));
    EXPECT_EQ(set.size, 1);
}
...
TEST(U64x2HashSetTest, AddDifferentElementsWithCommonLanes) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);
    ...
    ASSERT_TRUE(U64x2HashSetAdd(&set, key1));
    EXPECT_EQ(set.size, 1);

    ASSERT_TRUE(U64x2HashSetAdd(&set, key2));
    EXPECT_EQ(set.size, 2);

    ASSERT_TRUE(U64x2HashSetAdd(&set, key3));
    EXPECT_EQ(set.size, 3);
}

// Verifies that querying for an element that was never inserted correctly
// returns false, even if the set is populated.
TEST(U64x2HashSetTest, ContainsMissingElement) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);
    ...
    U64x2HashSetAdd(&set, key1);

    EXPECT_FALSE(U64x2HashSetContains(&set, key2));
}
...
TEST(U64x2HashSetTest, ZeroVector) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);

    U64x2 zero = {0, 0};

    EXPECT_TRUE(U64x2HashSetAdd(&set, zero));
    EXPECT_TRUE(U64x2HashSetContains(&set, zero));
    EXPECT_EQ(set.size, 1);
}

// Verifies that linear probing handles sequential additions properly without
// incorrectly overwriting elements.
TEST(U64x2HashSetTest, AddMultipleElements) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);
...
// factor.
TEST(U64x2HashSetTest, MaximumCapacity) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);
...
// probing loop.
TEST(U64x2HashSetTest, MissingElementInFullSet) {
    U64x2HashSet set;
    U64x2HashSetInit(&set);
    ...
    EXPECT_FALSE(U64x2HashSetContains(&set, missing_key));
}
