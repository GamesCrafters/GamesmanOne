...
// ------------------------------------------------------------------------
// Benchmark: U64x2HashSetAdd
// Measures the time it takes to add N items to an empty hash set.
// ------------------------------------------------------------------------
static void BM_U64x2HashSetAdd(benchmark::State& state) {
...
BENCHMARK(BM_U64x2HashSetAdd)->RangeMultiplier(2)->Range(8, kMaxElements);

// ------------------------------------------------------------------------
// Benchmark: U64x2HashSetContains (Hit)
// Measures the time to successfully find elements that exist in the set.
// ------------------------------------------------------------------------
static void BM_U64x2HashSetContains_Hit(benchmark::State& state) {
    size_t num_elements = state.range(0);
    std::vector<U64x2> keys = GenerateKeys(num_elements);
    ...
    for (const auto& key : keys) {
        U64x2HashSetAdd(&hs, key);
    }

    size_t idx = 0;
    for (auto _ : state) {
        // Sequentially lookup elements we know are in the set
        bool found = U64x2HashSetContains(&hs, keys[idx % num_elements]);
...
BENCHMARK(BM_U64x2HashSetContains_Hit)
    ->RangeMultiplier(2)
    ->Range(8, kMaxElements);

// ------------------------------------------------------------------------
// Benchmark: U64x2HashSetContains (Miss)
// Measures the time to reject elements that do not exist in the set.
// ------------------------------------------------------------------------
static void BM_U64x2HashSetContains_Miss(benchmark::State& state) {
    size_t num_elements = state.range(0);
    ...
    for (auto _ : state) {
        bool found =
            U64x2HashSetContains(&hs, keys_not_in_set[idx % num_elements]);
        benchmark::DoNotOptimize(found);
        ++idx;
    }
...
BENCHMARK(BM_U64x2HashSetContains_Miss)
    ->RangeMultiplier(2)
    ->Range(8, kMaxElements);
